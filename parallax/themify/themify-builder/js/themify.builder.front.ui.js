	...
	var ThemifyPageBuilder = {
		...
		bindEvents: function() {
			...
			if(this.is_touch_device()){
				...
				$body.on('touchend', '.themify_builder_module_front_overlay', function(e){
		...
		optionsModule: function(e) {
			...
			$.ajax({
				...
				success: function( data ){
					...
					$('.themify_builder_lightbox_title').text(module_name);
								...
								$this_option.find('.themify_builder_row').each(function(r){
									$(this).find('.tfb_lb_option_child').each(function(i){
										...
										if ( $this_option_child.hasClass('tfb_lb_wp_editor') && !$this_option_child.hasClass('clone') ) {
											...
											if ( typeof tinyMCE !== 'undefined' ) {
												tinyMCE.execCommand( "mceAddControl", true, this_option_id_real );
		...
		closeLightBox: function(e) {
			...
			setTimeout(function(){
				// hide the lightbox in timeout instead of animate callback to fix conflict with jquery.animate-enhanced.js
		...
		moduleSave: function(e){
			...
			$('#tfb_module_settings .tfb_lb_option').each(function(iterate){
				...
				else if( $(this).hasClass('themify_builder_row_js_wrapper') && $(this).is(':visible') ){
					var row_items = [];
					$(this).find('.themify_builder_row:visible').each(function(){
						var temp_rows = {};
				...
				else if ( $(this).is('select:visible, input:visible, textarea:visible') ) {
					option_value = $(this).val();
				}
			...
			$('#tfb_module_settings').find('.tfb_lb_wp_editor.clone').each(function(i){
				...
				$.ajax({
					...
					success: function( data ){
						...
						if ( typeof tinyMCE !== 'undefined' ) {
							tinyMCE.execCommand( "mceAddControl", true, this_option_id_clone );
