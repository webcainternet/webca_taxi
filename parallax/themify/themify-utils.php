...
/***************************************************************************/
	...
	/**
	 * Enqueues the chosen skin, if there was one selected.
	 * @since 1.4.2
	 */
	function themify_get_skin_stylesheet() {
...
function themify_meta_field_layout( $args, $call_before_after = true, $echo = true ) {
	...
	foreach ( $meta_box['meta'] as $options ) {
		...
		$ops_html .= sprintf('<a href="#" class="preview-icon %s"><img src="%s" title="%s" alt="%s" %s /></a>',
			$class, get_template_directory_uri()."/".$options['img'], $title, $options['value'],
...
function themify_get_uploader( $id = '', $args = array() ){
	...
	if( $medialib ) {
		...
		<div class="themify_medialib_wrapper">
			<?php echo $upload_visible? '&nbsp;<em>'.__('or', 'themify').'</em>': ''; ?> &nbsp; <a href="#" class="themify-media-lib-browse" data-submit='<?php echo json_encode($medialib_datas); ?>' data-uploader-title="<?php _e('Browse Image', 'themify') ?>" data-uploader-button-text="<?php echo $medialib_btn_text; ?>" data-fields="<?php echo $fields; ?>"><?php _e('Browse Library', 'themify') ?></a>
		</div>
		<?php
		$html .= ob_get_contents();
		ob_end_clean();
...
function themify_open_graph(){
	if('' != themify_check('setting-og_disable')) return;
	...
	if ( is_home() || is_front_page() ){
		...
		if( is_category() || is_tag() || is_tax() ){
			...
			if( ! $description ) {
				global $post;
				$description = isset( $post ) && isset( $post->post_content ) ? strip_tags($post->post_content) : '';
				$excerpt_length = 55;
...
if(!function_exists('themify_lightbox_vars_init')){
	...
	function themify_lightbox_vars_init(){
		...
		// Include prettyPhoto style and script
		...
		if('prettyphoto' == $gallery_lightbox || null == $gallery_lightbox){
